...
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.addon.tv.m3u.TvM3uEpgItem;
...
public class TvRootItem extends ItemContainer<TvM3uItem> implements TvItem {
	...
	private static final Pref<BooleanSupplier> FREE_TV_READY = Pref.b("FREE_TV_READY", false).withInheritance(false);
	...
	public FutureSupplier<? extends Item> getItem(@Nullable String scheme, String id) {
		...
		switch (scheme) {
			...
			case me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.addon.tv.m3u.TvRecentlyPlayedItem.SCHEME:
				return me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.addon.tv.m3u.TvRecentlyPlayedItem.create(this, id);
			default:
				return null;
		}
	}
	...
	protected FutureSupplier<String> buildTitle() {
		return completed(getLib().getContext().getString(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.addon_name_tv));
	}
	...
	public boolean isChildItemId(String id) {
		return id.startsWith(TvM3uTrackItem.SCHEME)
				...
				|| id.startsWith(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.addon.tv.m3u.TvRecentlyPlayedItem.SCHEME);
	}
	...
	private synchronized void ensureFreeTv() {
		if (getBooleanPref(FREE_TV_READY)) return;
		String path = BundledM3u.ensureLocal(getLib().getContext(), "world_free.m3u", "world_free.m3u");
		if (path == null) return;
		TvM3uFile file = TvM3uFileSystem.getInstance().createNewFile();
		file.setName("Free-TV");
		file.setUrl(path);
		file.setVideo(true);
		applyBooleanPref(FREE_TV_READY, true);
		addSource(file);
	}
