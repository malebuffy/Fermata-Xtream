...
public class PreferenceViewAdapter extends RecyclerView.Adapter<PreferenceViewAdapter.PrefViewHolder> {
	...
	public PrefViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
		...
		if (ctx.getTheme().resolveAttribute(R.attr.preferenceItemBackground, background, true) &&
				(background.resourceId != 0)) v.setBackgroundResource(background.resourceId);
		else v.setBackgroundResource(R.drawable.box_secondary);
