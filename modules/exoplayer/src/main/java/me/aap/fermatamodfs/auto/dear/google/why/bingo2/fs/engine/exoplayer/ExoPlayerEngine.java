...
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.addon.TranslateAddon.Translator;
...
public class ExoPlayerEngine extends MediaEngineBase implements Player.Listener {
	...
	static {
		...
		try {
			...
			if (cre != null) {
				factory = new CronetDataSource.Factory(cre, Executors.newSingleThreadExecutor());
			}
		} catch (Throwable err) {
			Log.w(err);
		}
		if (factory == null) factory = createDefaultHttpDataSourceFactory();
		httpDsFactory = factory;
	}
	...
	private final VisualizerAudioProcessor visualizerProc = new VisualizerAudioProcessor();
			...
			protected AudioSink buildAudioSink(@NonNull Context context,
				...
				return new DefaultAudioSink.Builder(ctx)
						...
						.setAudioProcessorChain(
								new DefaultAudioSink.DefaultAudioProcessorChain(visualizerProc, audioProc)).build();
			}
	...
	public void setVideoView(VideoView view) {
		...
		} else if (view.isTextureVideo()) {
			player.clearVideoSurface();
			player.setVideoTextureView(view.getVideoTexture());
		} else {
			player.clearVideoTextureView(view.getVideoTexture());
	...
	public void setVideoSurface(@Nullable android.view.Surface surface, int width, int height) {
		...
		PlayableItem item = getSource();
		int scale = (item == null) ? MediaPrefs.SCALE_BEST : item.getPrefs().getVideoScalePref();
		player.setVideoScalingMode((scale == MediaPrefs.SCALE_FILL) ?
	...
	private static DataSource.Factory createDefaultHttpDataSourceFactory() {
	...
	public float getVideoWidth() {
		...
		if (f == null) return 0;
		return f.width * f.pixelWidthHeightRatio;
	}
	...
	public void onVideoSizeChanged(VideoSize videoSize) {
		me.aap.utils.log.Log.i("ExoPlayer.onVideoSizeChanged: ", videoSize.width, "x", videoSize.height,
