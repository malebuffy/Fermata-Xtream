...
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.BrowsableItemPrefs.SORT_MASK_NAME_RND;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.ui.activity.MainActivityPrefs.getGridViewPrefKey;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.ui.activity.MainActivityPrefs.hasGridViewPref;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.ui.activity.MainActivityPrefs.hasTextIconSizePref;
...
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.lib.MediaLib.ArchiveItem;
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.lib.MediaLib.BrowsableItem;
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.lib.MediaLib.EpgItem;
...
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.ui.view.MediaItemListViewAdapter;
...
public abstract class MediaLibFragment extends MainActivityFragment implements MainActivityListener,
	...
	private boolean lastPlayedRestored;
	private int lastPlayedRestoreRetries;
	private static final int MAX_LAST_PLAYED_RESTORE_RETRIES = 30;
	private static final long LAST_PLAYED_RESTORE_RETRY_MS = 500L;
	private final Runnable restoreLastPlayedRunnable = this::restoreLastPlayedChannel;
	...
	public void revealItem(Item i) {
		...
		if (p == null) return;
		FutureSupplier<?> set = !p.equals(a.getParent()) ? a.setParent(p) : completedVoid();
		// Make sure the target folder has loaded before focusing the restored station.
		set.then(v -> p.getChildren()).main(getMainActivity().getHandler())
				.onSuccess(l -> getListView().focusTo(i));
	}
	...
	protected boolean isSupportedItemId(String id) {
		Item cached = getLib().getCachedItem(id);
		return (cached != null) && isSupportedItem(cached);
	}

	protected String getLastPlayedIdForRestore() {
		return getLib().getPrefs().getLastPlayedItemPref();
	}
	...
	protected int getLastPlayedRestoreMaxRetries() {
		return MAX_LAST_PLAYED_RESTORE_RETRIES;
	}

	protected void scheduleRestoreLastPlayedChannel() {
		if (!shouldRestoreLastPlayedChannel()) return;
		lastPlayedRestoreRetries = 0;
		App.get().getHandler().removeCallbacks(restoreLastPlayedRunnable);
		App.get().getHandler().post(restoreLastPlayedRunnable);
	}

	protected void resetRestoreLastPlayedChannel() {
		lastPlayedRestored = false;
		lastPlayedRestoreRetries = 0;
		App.get().getHandler().removeCallbacks(restoreLastPlayedRunnable);
	}

	private void restoreLastPlayedChannel() {
		if (lastPlayedRestored || isHidden()) return;
		if (getAdapter() == null) {
			retryRestoreLastPlayedChannel();
			return;
		}

		PlayableItem current = getMainActivity().getCurrentPlayable();
		if ((current != null) && isSupportedItem(current)) {
			lastPlayedRestored = true;
			revealItem(current);
			return;
		}

		String lastId = getLastPlayedIdForRestore();
		if ((lastId == null) || !isSupportedItemId(lastId)) {
			tryRestoreLastFolder();
			return;
		}

		String folderId = getLastFolderIdForRestore();
		if ((folderId != null) && isSupportedItemId(folderId)) {
			getLib().getItem(folderId).main(getMainActivity().getHandler()).onCompletion((folder, err) -> {
				if (lastPlayedRestored || isHidden()) return;
				if (folder instanceof BrowsableItem f && isSupportedItem(f)) {
					openItem(f);
					f.getChildren().main(getMainActivity().getHandler())
							.onCompletion((children, childrenErr) -> playRestoredChannel(lastId));
				} else {
					playRestoredChannel(lastId);
				}
			});
			return;
		}
	...
	private void playRestoredChannel(String lastId) {
		getLib().getItem(lastId).main(getMainActivity().getHandler()).onCompletion((item, err) -> {
			if (lastPlayedRestored || isHidden()) return;
			if (item instanceof PlayableItem last && isSupportedItem(last)) {
				lastPlayedRestored = true;
				getMainActivity().getMediaServiceBinder().playItem(last);
				revealItem(last);
			} else {
				retryRestoreLastPlayedChannel();
			}
	...
	private void tryRestoreLastFolder() {
		...
		getLib().getItem(folderId).main(getMainActivity().getHandler()).onCompletion((item, err) -> {
			if (lastPlayedRestored || isHidden()) return;
			if (item instanceof BrowsableItem folder && isSupportedItem(folder)) {
				openItem(folder);
				folder.getChildren().main(getMainActivity().getHandler())
						.onCompletion((children, childrenErr) -> lastPlayedRestored = true);
			} else {
				retryRestoreLastPlayedChannel();
			}
		});
	...
	private void retryRestoreLastPlayedChannel() {
		if (++lastPlayedRestoreRetries >= getLastPlayedRestoreMaxRetries()) {
			lastPlayedRestored = true;
			return;
		}
		App.get().getHandler().postDelayed(restoreLastPlayedRunnable, LAST_PLAYED_RESTORE_RETRY_MS);
	}
	...
	public class ListAdapter extends MediaItemListViewAdapter {
		...
		protected void setChildren(List<? extends Item> children) {
			...
			if (shouldRestoreLastPlayedChannel() && !lastPlayedRestored) {
				App.get().getHandler().removeCallbacks(restoreLastPlayedRunnable);
				App.get().getHandler().post(restoreLastPlayedRunnable);
			}
			if (noScroll) return;
