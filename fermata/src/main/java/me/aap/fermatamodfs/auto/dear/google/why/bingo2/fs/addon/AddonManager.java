...
public class AddonManager extends BasicEventBroadcaster<AddonManager.Listener>
	...
	private static final int LOAD_ADDON_RETRY_MAX = 8;
	private static final int LOAD_ADDON_RETRY_DELAY_MS = 500;
	...
	private synchronized boolean loadAddon(AddonInfo i) {
		...
		try {
			...
			Log.e(ex, "Failed to load addon class: ", i.className);
			return false;
		}
	}
	...
	private synchronized void scheduleLoadAddon(AddonInfo i, FutureSupplier<?> task, int counter) {
		App.get().getHandler().postDelayed(() -> {
			...
			if (loadAddon(i)) {
				task.cancel();
			} else if (counter >= LOAD_ADDON_RETRY_MAX) {
				String msg = "Failed to load addon after retries: " + i.className;
				Log.e(msg);
				if (task instanceof Promise) {
					((Promise<?>) task).completeExceptionally(new RuntimeException(msg));
				} else {
					task.cancel();
				}
			} else {
