...
public interface MainActivityPrefs
	...
	/** Custom theme shell/window background image URI (paid builds). Empty = theme default. */
	...
	Pref<IntSupplier> MAIN_MENU_COLOR_RED = Pref.i("MAIN_MENU_COLOR_RED", 255);
	Pref<IntSupplier> MAIN_MENU_COLOR_GREEN = Pref.i("MAIN_MENU_COLOR_GREEN", 255);
	Pref<IntSupplier> MAIN_MENU_COLOR_BLUE = Pref.i("MAIN_MENU_COLOR_BLUE", 255);
	...
	Pref<BooleanSupplier> VIDEO_IN_MOTION = AUTO ? Pref.b("VIDEO_IN_MOTION", false) : null;
	Pref<IntSupplier> VIDEO_IN_MOTION_THRESHOLD =
			AUTO ? Pref.i("VIDEO_IN_MOTION_THRESHOLD", 0) : null;
	Pref<BooleanSupplier> SPEED_DISPLAY = AUTO ? Pref.b("SPEED_DISPLAY", false) : null;
	...
	Pref<BooleanSupplier> VOICE_CONTROL_HOME_LISTEN = Pref.b("VOICE_CONTROL_HOME_LISTEN", true);
	Pref<BooleanSupplier> FIRST_RUN_SCAM_WARNING_SHOWN =
			Pref.b("FIRST_RUN_SCAM_WARNING_SHOWN", false);
	...
	Pref<IntSupplier> MAIN_MENU_COLOR_RED_AA = AUTO ? Pref.i("MAIN_MENU_COLOR_RED_AA", 255) : null;
	Pref<IntSupplier> MAIN_MENU_COLOR_GREEN_AA = AUTO ? Pref.i("MAIN_MENU_COLOR_GREEN_AA", 255) : null;
	Pref<IntSupplier> MAIN_MENU_COLOR_BLUE_AA = AUTO ? Pref.i("MAIN_MENU_COLOR_BLUE_AA", 255) : null;
	...
	static boolean hasThemePref(MainActivityDelegate a, List<Pref<?>> prefs) {
		if (prefs.contains(THEME_BACKGROUND_URI)) return true;
		if (AUTO && a.isCarActivity()) return prefs.contains(THEME_AA);
		return prefs.contains(THEME_MAIN);
	}

	@Nullable
	default String getThemeBackgroundUriPref() {
		String uri = getStringPref(THEME_BACKGROUND_URI);
		return (uri == null || uri.isBlank()) ? null : uri;
	}

	default void setThemeBackgroundUriPref(@Nullable String uri) {
		applyStringPref(THEME_BACKGROUND_URI, uri == null ? "" : uri);
	}
	...
	default int getMainMenuColorPref(MainActivityDelegate a) {
		if (AUTO && a.isCarActivity()) {
			return 0xff000000 | (getIntPref(MAIN_MENU_COLOR_RED_AA) << 16) |
					(getIntPref(MAIN_MENU_COLOR_GREEN_AA) << 8) | getIntPref(MAIN_MENU_COLOR_BLUE_AA);
		}
		return 0xff000000 | (getIntPref(MAIN_MENU_COLOR_RED) << 16) |
				(getIntPref(MAIN_MENU_COLOR_GREEN) << 8) | getIntPref(MAIN_MENU_COLOR_BLUE);
	}
	...
	default boolean getVideoInMotionPref() {
		return AUTO && getBooleanPref(VIDEO_IN_MOTION);
	}

	default int getVideoInMotionThresholdPref() {
		return AUTO ? getIntPref(VIDEO_IN_MOTION_THRESHOLD) : 0;
	}

	default boolean getSpeedDisplayPref() {
		return AUTO && getBooleanPref(SPEED_DISPLAY);
	}
	...
	default boolean getVoiceControlHomeListenPref() {
		return getBooleanPref(VOICE_CONTROL_HOME_LISTEN);
	}
