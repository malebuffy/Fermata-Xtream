...
public class YoutubeAddon extends WebBrowserAddon implements PreferenceStore.Listener {
	...
	private static final Pref<BooleanSupplier> YT_AUTOPLAY_ON_OPEN =
			Pref.b("YT_AUTOPLAY_ON_OPEN", false);
	...
	public void contributeSettings(Context ctx, PreferenceStore store, PreferenceSet set,
		...
		set.addBooleanPref(o -> {
			o.store = getPreferenceStore();
			o.pref = YT_AUTOPLAY_ON_OPEN;
			o.title = R.string.autoplay_on_open;
			o.visibility = visibility;
		});
	...
	boolean autoplayOnOpen() {
		return getPreferenceStore().getBooleanPref(YT_AUTOPLAY_ON_OPEN);
	}

	boolean autoplayOnOpenChanged(List<Pref<?>> prefs) {
		return prefs.contains(YT_AUTOPLAY_ON_OPEN);
	}
