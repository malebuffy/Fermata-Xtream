...
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.MEDIA_ENG_YT;
...
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.addon.web.yt.YoutubeAddon.VideoScale;
...
class YoutubeMediaEngine implements MediaEngine, OverlayMenu.SelectionHandler {
	...
	public YoutubeMediaEngine(YoutubeWebView web, MainActivityDelegate a) {
		...
		addonId = web.getAddon().getAddonId();
		String prefix = ID + ':' + addonId;
		currentId = prefix + ":current";
		mediaRoot = new ExtRoot(prefix, a.getLib());
		next = new YoutubeItem(prefix + ":next", mediaRoot, GenericFileSystem.getInstance().create("http://youtube.com/next"));
		prev = new YoutubeItem(prefix + ":prev", mediaRoot, GenericFileSystem.getInstance().create("http://youtube.com/prev"));
		end = new YoutubeItem(prefix + ":end", mediaRoot, GenericFileSystem.getInstance().create("http://youtube.com/end")) {
			@NonNull
			@Override
	...
	void playing(String url) {
		if (!web.allowPlaybackStart()) {
			// Autoplay is off and the user did not press play — stop site-started playback.
			web.pause();
			return;
		}
	...
	public void contributeToMenu(OverlayMenu.Builder b) {
		...
		b.addItem(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.video_scaling,
				ResourcesCompat.getDrawable(r, R.drawable.video_scaling, ctx.getTheme()),
				r.getString(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.video_scaling)).setSubmenu(this::videoScalingMenu);
	}
		...
		return web.getVideoQualities().timeout(1100).main()
				...
				.map(qualities -> {
					if ((qualities == null) || (qualities.isEmpty())) {
						b.addItem(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.auto, null, me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.auto)
	...
	private void videoScalingMenu(OverlayMenu.Builder b) {
		...
		b.addItem(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.video_scaling_best, null, me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.video_scaling_best)
				.setChecked(scale == VideoScale.CONTAIN, true);
		b.addItem(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.video_scaling_fill, null, me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.video_scaling_fill)
		...
		b.addItem(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.video_scaling_orig, null, me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.video_scaling_orig)
	...
	public boolean menuItemSelected(OverlayMenuItem item) {
		...
		if (itemId == me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.video_scaling_best) {
			web.setScale(VideoScale.CONTAIN);
			return true;
		} else if (itemId == me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.video_scaling_fill) {
			web.setScale(VideoScale.FILL);
			return true;
		} else if (itemId == R.id.video_scaling_fill_proportional) {
			web.setScale(VideoScale.COVER);
			return true;
		} else if (itemId == me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.video_scaling_orig) {
	...
	static boolean isYoutubeItem(MediaLib.Item i, int addonId) {
		return (i instanceof YoutubeItem) && i.getId().startsWith(ID + ':' + addonId + ':');
	}
	...
	private final class Current extends YoutubeItem {
		...
		public Current(String url) {
			super(currentId, mediaRoot, GenericFileSystem.getInstance().create(url));
		}
