...
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.engine.MetadataBuilder;
...
public abstract class BrowsableItemBase extends ItemBase implements BrowsableItem, BrowsableItemPrefs {
	...
	private FutureSupplier<List<Item>> sortChildren(List<Item> list) {
		...
		switch (prefs.getSortByPref()) {
			...
			case BrowsableItemPrefs.SORT_BY_ADDED:
				// Newest first by default; "descending" flips to oldest first.
				Collections.sort(sorted, (i1, i2) -> compareByAdded(i1, i2, !desc));
				break;
	...
	private int compareByAdded(Item i1, Item i2, boolean newestFirst) {
		long a1 = addedOf(i1);
		long a2 = addedOf(i2);
		int c = Long.compare(a1, a2);
		if (c == 0) return compareByName(i1, i2, false);
		return newestFirst ? -c : c;
	}

	private static long addedOf(Item i) {
		return (i instanceof M3uTrackItem) ? ((M3uTrackItem) i).getAddedMs() : 0L;
	}
