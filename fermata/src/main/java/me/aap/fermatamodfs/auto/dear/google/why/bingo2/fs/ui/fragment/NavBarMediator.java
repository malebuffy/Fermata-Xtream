...
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.BuildConfig.VERSION_CODE;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.BuildConfig.VERSION_NAME;
...
public class NavBarMediator extends PrefNavBarMediator
	...
	public void itemSelected(View item, int id, ActivityDelegate a) {
		if (id == R.id.menu) {
			...
			View active = ma.getNavBar().findViewById(ma.getActiveNavItemId());
	...
	@Override
	public void fragmentChanged(NavBarView nb, ActivityDelegate activity, ActivityFragment fragment) {
		super.fragmentChanged(nb, activity, fragment);
		int id = fragment.getFragmentId();
		if (nb.findViewById(id) != null) {
			((MainActivityDelegate) activity).rememberLastModule(id);
		}
	}
	...
	protected boolean extItemSelected(OverlayMenuItem item) {
		if (item.getItemId() == R.id.menu) {
			...
			if ((ext != null) && !ext.isSelected()) {
				...
				if (i != null) setExtButton(a.getNavBar(), i);
			}
		...
		if (navItem != null && nb != null) {
			setExtButton(nb, navItem);
		}

		a.openModule(id, launchInput(navItem));
		return true;
	}
	...
	public void itemReselected(View item, int id, ActivityDelegate a) {
		...
		if (b.isVideoMode()) {
			// Stremio: full browse UI. Other modules keep split + slider.
	...
	public void showMenu(MainActivityDelegate a) {
		...
		menu.show(b -> {
			...
			b.addItem(R.id.home_fragment, R.drawable.home_menu, R.string.main_menu);
	...
	public boolean menuItemSelected(OverlayMenuItem item) {
		...
		if (itemId == R.id.home_fragment) {
			MainActivityDelegate.get(item.getContext()).showMainMenu();
			return true;
		} else if (itemId == R.id.nav_got_to_current) {
			...
			f.setContentProvider(g -> {
				...
				v.setOnClickListener(t -> Utils.openUrl(t.getContext(), url));
				g.addView(v);
			});
			return true;
	...
	@Nullable
	private static Object launchInput(@Nullable NavBarItem item) {
		return null;
	}
