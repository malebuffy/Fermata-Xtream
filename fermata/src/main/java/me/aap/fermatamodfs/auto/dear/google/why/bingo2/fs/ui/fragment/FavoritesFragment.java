...
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.FavoritesPrefs;
...
public class FavoritesFragment extends MediaLibFragment {
	...
	private class FavoritesAdapter extends ListAdapter {
		...
		protected void onItemDismiss(int position) {
			if ((position < 0) || (position >= getList().size())) return;
		...
		protected boolean onItemMove(int fromPosition, int toPosition) {
			...
			if ((fromPosition < 0) || (toPosition < 0) ||
					(fromPosition >= size) || (toPosition >= size)) return false;
