...
public class ControlPanelView extends ConstraintLayout
	...
	public ControlPanelView(Context context, AttributeSet attrs) {
		...
		var surface = ta.getDrawable(R.styleable.ControlPanelView_surfaceBackground);
		if (surface != null) setBackground(surface);
		else setBackgroundColor(ta.getColor(R.styleable.ControlPanelView_android_colorBackground, 0));
		ta.recycle();
	...
	public void bind(FermataServiceUiBinder b) {
		...
		b.bindStopButton(findViewById(R.id.control_stop));
	...
	private void setSize(float scale) {
		...
		if (seek.isEnabled()) {
			...
			setHeight(R.id.control_stop, buttonSize);
	...
	private final class MenuHandler extends MediaItemMenuHandler {
		...
		private void buildAudioStreamMenu(OverlayMenu.Builder b) {
			...
			for (int i = 0; i < streams.size(); i++) {
				...
				b.addItem(android.view.View.generateViewId(), s.toString()).setData(s).setChecked(s.equals(ai));
			}
		}
		...
		protected void buildSubtitlesMenu(OverlayMenu.Builder b) {
			// Keep stream picker as normal OverlayMenu items (same look as audio tracks).
			// PreferenceSet uses a RecyclerView with different colors — nest it separately.
			b.addItem(R.id.select_subtitles, R.string.select_subtitles)
					.setFutureSubmenu(this::buildSubtitleStreamMenu);
			b.addItem(R.id.subtitle_prefs, R.string.subtitle_delay)
		...
		private void buildSubtitleDelayMenu(OverlayMenu.Builder b) {
			...
			SettingsFragment.addAutoSubPrefs(getContext(), prefSet, getItem().getPrefs(), null, false);
			SettingsFragment.addDelayPrefs(prefSet, getItem().getPrefs(),
					me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaLibPrefs.SUB_DELAY,
					R.string.subtitle_delay, null);
			SettingsFragment.addSubSizePrefs(prefSet, getItem().getPrefs(), null);
		...
		private FutureSupplier<Void> buildSubtitleStreamMenu(OverlayMenu.Builder b) {
			...
			b.setTitle(R.string.select_subtitles);
			return engine.getSubtitleStreamInfo().main().map(streams -> {
				...
				for (int i = 0; i < streams.size(); i++) {
					...
					// Unique ids — UiUtils.getArrayItemId only covers 0..9 and collapses the rest.
					b.addItem(android.view.View.generateViewId(), formatSubtitleLabel(s)).setData(s)
		...
		private static String formatSubtitleLabel(SubtitleStreamInfo s) {
			String desc = s.getDescription();
			String lang = s.getLanguage();
			if (desc != null && !desc.isBlank() && lang != null && !lang.isBlank()) {
				// Avoid "OpenSubtitles - [English]" double-bracket clutter from MediaStreamInfo.toString
