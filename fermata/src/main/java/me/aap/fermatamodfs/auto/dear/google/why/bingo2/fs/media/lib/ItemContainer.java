...
public abstract class ItemContainer<C extends Item> extends BrowsableItemBase {
	...
	FutureSupplier<List<Item>> listChildren(PreferenceStore prefs, Pref<Supplier<String[]>> idsPref) {
		...
		return loadChildren(prefs, idsPref, ids, true);
	}

	private FutureSupplier<List<Item>> loadChildren(PreferenceStore prefs,
																									Pref<Supplier<String[]>> idsPref,
																									String[] ids,
																									boolean retryIfEmpty) {
		MediaLib lib = getLib();
		List<Item> children = new ArrayList<>(ids.length);
		// Keep unresolved ids — cold-start races (radio/TV M3U still loading) must not wipe lists.
		String[] nextIds = Arrays.copyOf(ids, ids.length);
		AtomicBoolean changed = new AtomicBoolean();
		AtomicInteger index = new AtomicInteger();
		return forEach(id -> {
			int i = index.getAndIncrement();
			return resolveChild(lib, id).map(resolved -> {
				if (resolved == null) {
					Log.w("Unresolved container item (kept): ", id);
					return null;
				}
				children.add(resolved.child);
				if (!id.equals(resolved.saveId)) {
					Log.i("Item id has been changed. Updating ", id, " -> ", resolved.saveId);
					nextIds[i] = resolved.saveId;
					changed.set(true);
				}
				return null;
			});
		}, ids).main().then(v -> {
			if (retryIfEmpty && children.isEmpty() && (ids.length > 0)) {
				return loadChildren(prefs, idsPref, ids, false);
			}
			if (changed.get()) {
				prefs.applyStringArrayPref(idsPref, nextIds);
			}
	...
	@NonNull
	private FutureSupplier<ResolvedChild> resolveChild(@NonNull MediaLib lib, @NonNull String id) {
		FutureSupplier<? extends Item> s;
		try {
			s = lib.getItem(id);
		} catch (Throwable err) {
		...
		return s.ifFail(err -> {
			...
			try {
				Item child = toChildItem(c);
				String saveId = (c instanceof PlayableItem) ? ((PlayableItem) c).getOrigId() : c.getId();
				if (saveId == null || saveId.isBlank()) saveId = c.getId();
				return new ResolvedChild(child, saveId);
			} catch (Throwable err) {
				Log.e(err, "Failed to import container item: ", id);
				return null;
			}
		});
	}
	...
	public FutureSupplier<Void> moveItem(int fromPosition, int toPosition) {
		return list().map(list -> {
			if ((fromPosition < 0) || (toPosition < 0) ||
					(fromPosition >= list.size()) || (toPosition >= list.size())) {
				return null;
			}
			List<C> newChildren = new ArrayList<>(list);
	...
	private static final class ResolvedChild {
		final Item child;
		final String saveId;

		ResolvedChild(Item child, String saveId) {
			this.child = child;
			this.saveId = saveId;
		}
	}
