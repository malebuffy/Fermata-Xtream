...
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.lib.MediaLib.StreamItem.STREAM_END_TIME;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.lib.MediaLib.StreamItem.STREAM_START_TIME;
...
public class FermataServiceUiBinder extends BasicEventBroadcaster<FermataServiceUiBinder.Listener>
	...
	private PlayableItem lastStoppedItem;
	...
	@Nullable
	public PlayableItem getLastStoppedItem() {
	...
	public void bindStopButton(View v) {
		stopButton = v;
		v.setOnClickListener(b -> stop());
	}
	...
	public void unbind() {
		...
		unbindButtons(playPauseButton, stopButton, prevButton, nextButton, rwButton, ffButton);
		playPauseButton = stopButton = prevButton = nextButton = rwButton = ffButton = null;
	...
	private final class MediaControllerCallback extends MediaControllerCompat.Callback {
		...
		public void onPlaybackStateChanged(PlaybackStateCompat state) {
			...
			if (playbackState == PlaybackStateCompat.STATE_STOPPED) {
				PlayableItem stopped = sessionCallback.getCurrentItem();
				if (stopped == null) stopped = currentItem;
				if (stopped != null) lastStoppedItem = stopped;
			} else if ((playbackState == PlaybackStateCompat.STATE_PAUSED) ||
					(playbackState == STATE_PLAYING) || (playbackState == STATE_BUFFERING)) {
				lastStoppedItem = null;
			}
			...
			switch (playbackState) {
				...
				case PlaybackStateCompat.STATE_BUFFERING:
					// Buffering is an active playback state. Some live/video engines can
					// already render while they still report BUFFERING, so keep the
					// control actionable as Pause instead of leaving its stale Play icon.
					playPause(STATE_PLAYING);
					showPanel(true);
					break;
				...
				case PlaybackStateCompat.STATE_STOPPED:
					...
					resetProgressBar();
					setPlayPauseButtonState(R.drawable.play, true, false);
					showPanel(false);
					break;
		...
		private void playPause(int st) {
			...
			if ((eng != null) && ((i = eng.getSource()) != null)) {
				if (st == STATE_PLAYING) {
					setPlayPauseButtonState(eng.canPause() ? R.drawable.pause : R.drawable.stop,
							false, eng.canPause());
				} else {
					setPlayPauseButtonState(R.drawable.play, true, false);
				}
		...
		private void playPause(MediaEngine eng, int st, int dur, int pos) {
			...
			if (st == STATE_PLAYING) {
				...
				if (playPauseButton != null) {
					if (eng.canPause()) {
						setPlayPauseButtonState(R.drawable.pause, false, true);
					} else {
						setPlayPauseButtonState(R.drawable.stop, false, false);
					}
				}
			} else {
				stopProgressUpdate();
		...
		private void setPlayPauseButtonState(int icon, boolean selected, boolean activated) {
			View b = playPauseButton;
			if (b == null) return;
			b.setSelected(selected);
			b.setActivated(activated);
			if (b instanceof ImageView) ((ImageView) b).setImageResource(icon);
		}

		private void updateStopButtonVisibility() {
			if (stopButton == null) return;
			MediaEngine eng = sessionCallback.getEngine();
			stopButton.setVisibility(((eng != null) && eng.canPause()) ? VISIBLE : GONE);
		}

		private void showPanel(boolean show) {
			if (controlPanel == null) return;
			MainActivityDelegate a = MainActivityDelegate.get(controlPanel.getContext());
			if (a.isHomeFragmentActive()) show = false;
