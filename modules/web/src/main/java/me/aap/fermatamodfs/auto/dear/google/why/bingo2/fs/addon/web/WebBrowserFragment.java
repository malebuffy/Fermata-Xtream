...
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.addon.web.FermataWebClient.isYoutubeKidsUri;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.addon.web.FermataWebClient.isYoutubeMusicUri;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.addon.web.FermataWebClient.isYoutubeUri;
...
public class WebBrowserFragment extends MainActivityFragment
	...
	public int getFragmentId() {
		return me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.web_browser_fragment;
	}
	...
	public void updateBrowserBar(String uri, boolean back, boolean forward) {
		MainActivityDelegate a = getActivityDelegate();
		ToolBarView tb = a.getToolBar();
		if (tb == null) return;
		WebToolBarMediator m = WebToolBarMediator.getInstance();
		m.setAddress(tb, uri);
		m.setButtonsVisibility(tb, back, forward);
	}
	...
	public void loadUrl(String url) {
		...
		if (v != null) {
			...
			if (!(this instanceof YoutubeFragment) && isYoutubeMusicUri(uri) &&
					AddonManager.get().hasAddon(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.youtube_music_fragment)) {
				String u = url;
			...
			} else if (!(this instanceof YoutubeFragment) && isYoutubeKidsUri(uri) &&
					AddonManager.get().hasAddon(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.youtube_kids_fragment)) {
				String u = url;
				MainActivityDelegate.getActivityDelegate(requireContext()).onSuccess(a -> {
			...
			} else if (!(this instanceof YoutubeFragment) && isYoutubeUri(uri) &&
					AddonManager.get().hasAddon(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.youtube_fragment)) {
				String u = url;
				MainActivityDelegate.getActivityDelegate(requireContext()).onSuccess(a -> {
	...
	public CharSequence getTitle() {
		return getString(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.addon_name_web_browser);
	}
	...
	public void contributeToNavBarMenu(OverlayMenu.Builder b) {
		...
		b.addItem(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.refresh,
				ResourcesCompat.getDrawable(res, me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.drawable.refresh, theme),
				res.getString(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.refresh)).setHandler(this);
		...
		b.addItem(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.bookmarks,
				ResourcesCompat.getDrawable(res, me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.drawable.bookmark_filled, theme),
				res.getText(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.bookmarks)).setSubmenu(this::bookmarksMenu);
	}
	...
	public void bookmarksMenu(OverlayMenu.Builder b) {
		...
		b.addItem(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.bookmark_create, me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.create_bookmark)
	...
	private void createBookmark(OverlayMenu.Builder b) {
		...
		set.addStringPref(o -> {
			o.store = store;
			o.pref = name;
			o.title = me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.bookmark_name;
		});
		set.addStringPref(o -> {
			o.store = store;
			o.pref = url;
	...
	private boolean bookmarkSelected(OverlayMenuItem item) {
		if (item.isLongClick()) {
			...
			item.getMenu().show(b ->
					b.addItem(me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.bookmark_remove, me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.string.remove_bookmark)
							.setHandler(i -> {
								WebBrowserAddon a = getAddon();
			...
			loadUrl(getBookmarkUrl(item.getData()));
		}

		return true;
	}

	protected String getBookmarkUrl(String url) {
		if (this instanceof YoutubeFragment) return url;
		return (Uri.parse(url).getScheme() == null) ? "https://" + url : url;
	}
			...
			if (a != null) {
				for (Map.Entry<String, String> e : a.getBookmarks().entrySet()) {
					if (q.equalsIgnoreCase(e.getValue())) {
						loadUrl(getBookmarkUrl(e.getKey()));
						return;
					}
				}
			}
