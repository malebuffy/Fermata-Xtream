...
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.engine.MediaEngine.NO_SUBTITLES;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.AE_ENABLED;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.BASS_ENABLED;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.BASS_STRENGTH;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.EQ_BANDS;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.EQ_ENABLED;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.EQ_PRESET;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.EQ_USER_PRESETS;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.VIRT_ENABLED;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.VIRT_MODE;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.VIRT_STRENGTH;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.VOL_BOOST_ENABLED;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.VOL_BOOST_STRENGTH;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.MediaPrefs.getUserPresetBands;
import static me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.pref.PlaybackControlPrefs.getTimeMillis;
...
import me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.lib.MediaLib.Favorites;
...
public class MediaSessionCallback extends MediaSessionCompat.Callback
	...
	public static final String EXTRA_POS = "me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.extra.pos";
	private static final String CUSTOM_ACTION_RW = "me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.action.rewind";
	private static final String CUSTOM_ACTION_FF = "me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.action.fastForward";
	private static final String CUSTOM_ACTION_REPEAT_ENABLE = "me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.action.repeat.enable";
	private static final String CUSTOM_ACTION_REPEAT_DISABLE =
			"me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.action.repeat" + ".disable";
	private static final String CUSTOM_ACTION_SHUFFLE_ENABLE =
			"me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.action.shuffle" + ".enable";
	private static final String CUSTOM_ACTION_SHUFFLE_DISABLE =
			"me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.action.shuffle.disable";
	private static final String CUSTOM_ACTION_FAVORITES_ADD = "me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.action.favorites.add";
	private static final String CUSTOM_ACTION_FAVORITES_REMOVE =
			"me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.action.favorites.remove";
	private static final int MAX_STREAM_END_RETRIES = 3;
	...
	private String streamEndRetryId;
	private int streamEndRetryCount;
	@NonNull
	private PlaybackStateCompat currentState;
	private Queue<Prioritized<VideoView>> videoView;
	private android.view.Surface externalSurface;
	private int externalSurfaceWidth;
	private int externalSurfaceHeight;
	...
	public void setExternalSurface(android.view.Surface surface, int width, int height) {
		this.externalSurface = surface;
		this.externalSurfaceWidth = width;
		this.externalSurfaceHeight = height;
		MediaEngine eng = getEngine();

		if (eng != null) {
			if (surface != null) {
				eng.setVideoSurface(surface, width, height);
			} else {
				eng.setVideoSurface(null, 0, 0);
				VideoView vv = getVideoView();
				if (vv != null) eng.setVideoView(vv);
			}
		}
	}
	...
	public void addVideoView(VideoView view, int priority) {
		...
		if (eng != null) {
			...
			if (i.isVideo() && (externalSurface == null)) eng.setVideoView(getVideoView());
		}
	}
	...
	public FutureSupplier<Void> play() {
		...
		switch (state.getState()) {
			...
			case STATE_PAUSED -> {
				...
				float speed = getSpeed(eng.getSource());

				if (externalSurface != null) {
					eng.setVideoSurface(externalSurface, externalSurfaceWidth, externalSurfaceHeight);
				}
	...
	private FutureSupplier<?> onStop(MediaEngine eng, long pos) {
		boolean currentEngine = (eng == engine);
		if (eng != null) {
			if (pos != -1) {
				PlayableItem i = eng.getSource();
		...
		// An engine replacement may have started while the old engine's asynchronous
		// position lookup was still completing. Do not let that stale stop publish
		// STATE_STOPPED over the replacement engine's STATE_PLAYING state.
		if ((eng == null) || currentEngine) stopped();
		return completedVoid();
	}
	...
	private FutureSupplier<?> engineEnded(MediaEngine engine) {
		...
		if (i != null) {
			if (i instanceof StreamItem) {
				String id = i.getId();
				if (!id.equals(streamEndRetryId)) {
					streamEndRetryId = id;
					streamEndRetryCount = 0;
				}

				if (streamEndRetryCount++ < MAX_STREAM_END_RETRIES) {
					Log.w("Failed to play stream? Retrying ", i);
					playItem(i, 0, true);
					return playerTask;
				}

				String msg = lib.getContext().getResources().getString(R.string.err_failed_to_play, i);
				Log.w(msg);
				PlaybackStateCompat state = new PlaybackStateCompat.Builder().setActions(SUPPORTED_ACTIONS)
						.setState(STATE_ERROR, 0, 1.0f)
						.setErrorMessage(PlaybackStateCompat.ERROR_CODE_UNKNOWN_ERROR, msg).build();
	...
	public void playItem(PlayableItem i, long pos) {
		playItem(i, pos, false);
	}

	public void playItemFresh(PlayableItem i, long pos) {
		playerTask.cancel();
		onStop(false).main().onCompletion((res, err) -> playItem(i, pos));
	}

	private void playItem(PlayableItem i, long pos, boolean retry) {
		playerTask.cancel();
		if (!retry) {
			streamEndRetryId = i.getId();
			streamEndRetryCount = 0;
		}
		setLastPlayed(i, pos);
		PlaybackStateCompat state = new PlaybackStateCompat.Builder().setActions(SUPPORTED_ACTIONS)
	...
	private void playPreparedItem(MediaEngine eng, PlayableItem i, long pos, PlayableItem current,
		...
		if (i.isVideo() && (videoView != null)) engine.setVideoView(getVideoView());
		if (i.isVideo() && (externalSurface != null)) engine.setVideoSurface(externalSurface, externalSurfaceWidth, externalSurfaceHeight);
	...
	private void setLastPlayed(PlayableItem i, long position) {
		...
		me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.media.PlayHistoryHooks.notifyPlayed(i);
