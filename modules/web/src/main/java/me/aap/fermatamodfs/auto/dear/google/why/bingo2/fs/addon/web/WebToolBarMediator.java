...
public class WebToolBarMediator implements ToolBarView.Mediator {
	...
	public void enable(ToolBarView tb, ActivityFragment f) {
		...
		addButton(tb, me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.drawable.bookmark_filled, v ->
				onBookmarksButtonClick(b), me.aap.fermatamodfs.auto.dear.google.why.bingo2.fs.R.id.bookmarks);
	...
	public void setButtonsVisibility(ToolBarView tb, boolean back, boolean forward) {
		View backBtn = tb.findViewById(me.aap.utils.R.id.tool_bar_back_button);
		if (backBtn != null) backBtn.setVisibility(back ? VISIBLE : GONE);
		View forwardBtn = tb.findViewById(R.id.browser_forward);
		if (forwardBtn != null) forwardBtn.setVisibility(forward ? VISIBLE : GONE);
	}

	private EditText createAddress(ToolBarView tb, WebBrowserFragment f) {
		EditText t = createEditText(tb);
		setLayoutParams(t, 0, WRAP_CONTENT).horizontalWeight = 2;
